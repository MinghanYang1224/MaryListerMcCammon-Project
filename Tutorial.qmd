...
As mentioned in Section 1, the log-likelihood can be calculated given the hazard function:
 \begin{equation}
    \ell (\theta) = \sum_{i=1}^n \delta_i \log h(t_i \mid \theta, Y_0) - \sum_{i=1}^n H(t_i \mid \theta, Y_0).
\end{equation}
For the hazard-response model, an analytical solution is not available. With an ODE solver from `DifferentialEquations.jl`[@Julia_differential_equations], we will be able to solve for the hazard function, the response function, and the cumulative hazard function numerically. 
```{julia}
#| output: false
...
$$
Then we can calculate the predictive hazard function 
$$
...
function plot_posterior_with_prior(postsamples, priors, param_idx, param_name, x_range)
    ...
    xlabel!(param_name)
    ylabel!("Density")
    title!("Posterior and Prior for $param_name",titlefont=font(10))
end
...
# Summary
Future work, adding covariates. q
