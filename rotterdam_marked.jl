...
df = DataFrame(time=collect(df_full.dtime) ./ 365.25, # take the time points
    status=collect(df_full.death)) # take the death indicator column 
...
#= Vital status =#
status = collect(Bool, (df.status)); #0，1，1，0，。。。convert the index 0 and 1 to boolean 
...
# Hazard-Response model with explicit Jacobian in Log scale
...
mlog_lik = function (par::Vector{Float64})
    if any(par .> 5.0)
        ...
        odeparams = exp.(par) #take exponential of the parameters

        sol = solve(ODEProblem(HRJ, u0, [0.0, tmax], odeparams); alg_hints=[:stiff])
        #     sol = solve(ODEProblem(HRJ, u0, tspan0[i, :], odeparams[i, :]), Tsit5())
        OUT = sol(df.time) # get the values at the time points


        # Terms in the log log likelihood function
        ll_haz = sum(log.(OUT[1, status])) # log likelihood of the observed events (death) part

        ll_chaz = sum(OUT[3, :]) # log likelihood of the censored data, this is the cumulative hazard.
...
#use these parameter values to solve ODE
